...
function AnchorCard({ song, onArtistFilter, artistFilter, onJumpToKey, onScrollTo, onCollapse }) {
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
        {onCollapse && (
          <span
            onClick={onCollapse}
            title="Collapse"
            style={{ fontSize: 11, color: '#5a6a8a', cursor: 'pointer', userSelect: 'none', padding: '3px 4px 0 0', flexShrink: 0 }}
            onMouseEnter={e => { e.currentTarget.style.color = '#00c266'; }}
...
function MatchRow({ match, anchor, selected, pinned, onClick, onNavigate, onScrollTo, onArtistFilter, onJumpToKey }) {
  ...
  const handleRowClick = onClick ? (e) => {
    // Delay single-click so double-click on key badge can cancel it
    clickTimer.current = setTimeout(() => onClick(e), 220);
  } : undefined;
    ...
    >
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
          <div
            onClick={onNavigate ? (e) => { e.stopPropagation(); onNavigate(other); } : (e) => { e.stopPropagation(); }}
            style={{
        ...
        <div style={{ marginTop: 4, display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <KeyBadge keyName={other.key} onDoubleClick={onJumpToKey ? e => { e.stopPropagation(); cancelRowClick(); onJumpToKey(other.key); } : undefined} />
        </div>
      </div>
...
function GroupHeader({ song, count, expanded, onToggle, onTitleClick, highlighted, sortControls, onArtistFilter, artistFilter }) {
  const artistSpans = (fontSize, baseColor) => song.artist.split(/;\s*|,\s+(?=[A-Z])/).map((a, ai, arr) => (
    <span key={ai} style={{ display: 'inline-flex', alignItems: 'center', gap: 2 }}>
      <span
        onClick={e => { e.stopPropagation(); onArtistFilter && onArtistFilter(a.trim()); }}
        style={{ fontSize, color: artistFilter === a.trim() ? '#00c266' : baseColor, cursor: onArtistFilter ? 'pointer' : 'default' }}
        onMouseEnter={onArtistFilter ? e => { e.currentTarget.style.color = '#00c266'; e.currentTarget.style.textDecoration = 'underline'; } : undefined}
        onMouseLeave={onArtistFilter ? e => { e.currentTarget.style.color = artistFilter === a.trim() ? '#00c266' : baseColor; e.currentTarget.style.textDecoration = 'none'; } : undefined}
      >{a.trim()}</span>
      {artistFilter === a.trim() && onArtistFilter && (
        <span onClick={e => { e.stopPropagation(); onArtistFilter(null); }} style={{ fontSize, color: '#00c266', cursor: 'pointer' }}>×</span>
      )}
      {ai < arr.length - 1 && <span style={{ fontSize, color: '#4a5a7a' }}>,</span>}
    </span>
  ));
  if (expanded) {
    return (
      <div style={{ borderBottom: '1px solid #1a1a22' }}>
        {/* Card with ▼ absolutely positioned in the left margin gap */}
        <div style={{ position: 'relative', margin: '12px 20px 0' }}>
          <span
            onClick={onToggle}
            title="Collapse"
            style={{ position: 'absolute', left: -16, top: 3, fontSize: 11, color: '#5a6a8a', cursor: 'pointer', userSelect: 'none' }}
            onMouseEnter={e => { e.currentTarget.style.color = '#00c266'; }}
            onMouseLeave={e => { e.currentTarget.style.color = '#5a6a8a'; }}
          >▼</span>
          <div style={{
            ...
            <div style={{ marginTop: 2, display: 'flex', flexWrap: 'wrap', gap: '0 4px' }}>{artistSpans(12, '#7a8aaa')}</div>
            <div style={{ marginTop: 8, display: 'flex', gap: 8, alignItems: 'center' }}>
              <span style={{ fontSize: 12, color: '#aaa', fontFamily: 'monospace' }}>{song.bpm} BPM</span>
              <KeyBadge keyName={song.key} />
              {song.relativeKey && <span style={{ fontSize: 11, color: '#6a7a9a' }}>rel. <KeyBadge keyName={song.relativeKey} /></span>}
            </div>
          </div>
        </div>
        ...
        {sortControls && (
          <div style={{
            ...
            <SortHeader label={`${count} Compatible Track${count !== 1 ? 's' : ''}`} sortKey="az" current={sortControls.matchSort} onToggle={k => sortControls.toggleMatchSort(k)} />
            <SortHeader label="Match" sortKey="score" current={sortControls.matchSort} onToggle={k => sortControls.toggleMatchSort(k)} align="center" invertArrow={true} />
            <SortHeader label="BPM Diff" sortKey="bpm" current={sortControls.matchSort} onToggle={k => sortControls.toggleMatchSort(k)} align="center" invertArrow={true} />
            <SortHeader label="Key" sortKey="key" current={sortControls.matchSort} onToggle={k => sortControls.toggleMatchSort(k)} align="right" />
          </div>
        )}
      </div>
    );
  }
  // Collapsed: compact row with ▶
  return (
    <div
      ...
      style={{
        ...
        background: highlighted ? '#00c26610' : '#13131a',
        borderBottom: '1px solid #1e1e28',
        borderTop: highlighted ? '1px solid #00c26630' : '1px solid #1e1e28',
        boxShadow: highlighted ? 'inset 2px 0 0 #00c266' : 'none',
        cursor: 'pointer', userSelect: 'none', transition: 'all 0.15s',
      }}
      onMouseEnter={e => { if (!highlighted) e.currentTarget.style.background = '#1a1a24'; }}
      onMouseLeave={e => { if (!highlighted) e.currentTarget.style.background = '#13131a'; }}
    >
      <span style={{ fontSize: 11, color: '#5a6a8a', flexShrink: 0, width: 20, textAlign: 'center' }}>▶</span>
      <div style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: highlighted ? '#00c266' : '#d8d8e0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{song.song}</span>
        <span style={{ marginLeft: 8, display: 'inline-flex', flexWrap: 'wrap', gap: '0 3px' }}>{artistSpans(11, '#6a7a9a')}</span>
      </div>
...
export default function RemixMatcher() {
  ...
  const cycleSort = useCallback((sortKey, firstDir, secondDir) => {
    setActiveSortList(prev => {
      ...
      // A-Z ↔ BPM are mutually exclusive: swap one for the other, keep KEY in its position
      ...
      if (conflictsWithAz || azConflictsWithBpm) {
        // Replace the outgoing sort with the incoming one in the same slot
        return prev.map(s => (s.key === 'az' || s.key === 'bpm') ? { key: sortKey, dir: firstDir } : s);
      }
      // A-Z always sits at the END (tiebreaker); KEY and BPM append in click order (first clicked = primary)
      if (sortKey === 'az') {
        ...
        // KEY always primary, BPM always secondary, A-Z always last — regardless of click order
        const az = prev.find(s => s.key === 'az');
        const keyEntry = sortKey === 'key' ? { key: 'key', dir: firstDir } : prev.find(s => s.key === 'key');
        const bpmEntry = sortKey === 'bpm' ? { key: 'bpm', dir: firstDir } : prev.find(s => s.key === 'bpm');
        const ordered = [keyEntry, bpmEntry].filter(Boolean);
        return az ? [...ordered, az] : ordered;
      }
    });
  }, []);
  ...
  const selectSong = useCallback((song) => {
    // Close any currently expanded group, open this song's group, highlight in col A
    ...
    if (songItemRefs.current[song.song]) {
      songItemRefs.current[song.song].scrollIntoView({ block: 'start', behavior: 'smooth' });
    }
    setSelectedMatch(null);
  }, []);
  ...
  const suggest = useCallback(() => {
    ...
    if (!suggestQueueRef.current.length) {
      // Helper: get the set of artists for a song
      const getArtists = s => s.artist.split(/;\s*|,\s+(?=[A-Z])/).map(a => a.trim().toLowerCase()).filter(Boolean);
      // Strip common suffixes like "(2023 Remaster)", "(Live)", "(Radio Edit)" etc. for title comparison
      const coreTitle = s => s.song.toLowerCase().replace(/\s*[\(\[][^\)\]]*[\)\]]/g, '').trim();
      // Filter out matches where the two songs share an artist AND have the same core title (remasters, alternate versions)
      const dedupedMatches = matches.filter(m => {
        const aArtists = getArtists(m.a);
        const bArtists = getArtists(m.b);
        const shareArtist = aArtists.some(a => bArtists.includes(a));
        const sameTitle = coreTitle(m.a) === coreTitle(m.b);
        return !(shareArtist && sameTitle);
      });
      const pool = dedupedMatches.length > 0 ? dedupedMatches : matches;
    ...
    const pairKey = [pick.m.a.song, pick.m.b.song].sort().join('|||');
    // Open anchor's group in col B, highlight in col A
    setExpandedGroups(prev => {
      const next = {};
      Object.keys(prev).forEach(k => { next[k] = false; });
      next[anchor.song] = true;
      return next;
    });
    setHighlightedSong(anchor);
    if (songItemRefs.current[anchor.song]) {
      songItemRefs.current[anchor.song].scrollIntoView({ block: 'start', behavior: 'smooth' });
    }
    // Pin the suggested match so both songs highlight
    setSelectedMatch('pair:' + pairKey);
  ...
  useEffect(() => {
    if (selectedSong && songItemRefs.current[selectedSong.song]) {
      songItemRefs.current[selectedSong.song].scrollIntoView({ block: 'start', behavior: 'smooth' });
    }
  }, [selectedSong]);
  ...
  const filteredSongs = songs
    ...
    .sort((a, b) => {
      ...
      // Multi-sort using activeSortList (index 0 = primary = first clicked)
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', height: 'calc(100vh - 65px)' }}>
        ...
        {songs.length > 0 && <div style={{
          width: colAWidth, minWidth: 220, flexShrink: 0,
          ...
          <div ref={songListRef} style={{ flex: 1, overflowY: 'auto' }}>
            {songs.length === 0 ? (
              ...
              filteredSongs.map((song, i) => {
                const isSelected = highlightedSong?.song === song.song;
          ...
          {/* Drag handle — tab pill on the border */}
          <div
            onMouseDown={e => {
              e.preventDefault();
            ...
            onMouseEnter={e => { e.currentTarget.style.background = '#00c26630'; e.currentTarget.style.borderColor = '#00c26660'; }}
            onMouseLeave={e => { e.currentTarget.style.background = '#1e2638'; e.currentTarget.style.borderColor = '#2a3348'; }}
          >
            <div style={{
              display: 'flex', flexDirection: 'column', gap: 3, pointerEvents: 'none',
            }}>
              {[0,1,2].map(i => <div key={i} style={{ width: 2, height: 2, borderRadius: '50%', background: '#5a6a8a' }} />)}
            </div>
          </div>
        </div>}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {songs.length > 0 && <div style={{
            ...
            {(() => {
              ...
              const expandedSongName = Object.keys(expandedGroups).find(k => expandedGroups[k]);
              const expandedSong = expandedSongName
                ? (() => { const groupMap = new Map(); displayedMatches.forEach(m => { [m.a, m.b].forEach(s => { if (!groupMap.has(s.song)) groupMap.set(s.song, { song: s, count: 0 }); }); const key = m.a.song; if (groupMap.has(key)) groupMap.get(key).count++; }); return groupMap.get(expandedSongName); })()
                : null;
              const label = expandedSong ? `Matches for "${expandedSongName}"` : 'All Matches';
              const count = expandedSong ? expandedSong.count : displayedMatches.length;
              return (
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
                  <span style={{ fontSize: 14, fontWeight: 600, color: '#d0d0d8' }}>{label}</span>
                  <span style={{ fontSize: 22, fontWeight: 800, fontFamily: 'monospace', color: '#00c266', letterSpacing: -1 }}>{count}</span>
                  <span style={{ fontSize: 12, color: '#00c26688', fontWeight: 600 }}>match{count !== 1 ? 'es' : ''}</span>
                </div>
              );
            })()}
          ...
          {/* Column headers — only in anchor mode */}
          {selectedSong && displayedMatches.length > 0 && (
            <div style={{
              position: 'relative',
              display: 'grid',
              gridTemplateColumns: '1fr 70px 90px 90px',
              ...
              <SortHeader label={`${displayedMatches.length} Compatible Track${displayedMatches.length !== 1 ? 's' : ''}`} sortKey="az" current={matchSort} onToggle={k => toggleSort(matchSort, k, setMatchSort)} />
          ...
          {selectedSong && (
            <div style={{ flexShrink: 0, borderBottom: '1px solid #1a1a22' }}>
              <AnchorCard song={selectedSong} onArtistFilter={artist => { setArtistFilter(prev => prev === artist ? null : artist); if (songListRef.current) songListRef.current.scrollTo({ top: 0, behavior: 'smooth' }); }} artistFilter={artistFilter} onJumpToKey={jumpToKey} onScrollTo={scrollToSong} onCollapse={clearSelection} />
            </div>
          )}
          ...
          {/* Expanded group: fills column B. Header fixed, matches scrollable. No other groups visible. */}
          {!selectedSong && (() => {
            ...
            const expandedEntry = [...groupMap.values()].find(({ song }) => expandedGroups[song.song] === true);
            if (!expandedEntry) return null;
            const { song, matches: rawGroupMatches } = expandedEntry;
            // Score and sort group matches by matchSort
            const scoredGroupMatches = rawGroupMatches.map(({ match, anchor }) => ({
              match: { ...match, score: scoreMatch(match.a, match.b, vibeWeight, sonicsWeight, utilityWeight) },
              anchor,
              pairKey: [match.a.song, match.b.song].sort().join('|||'),
            }));
            const sortedGroupMatches = [...scoredGroupMatches].sort((a, b) => {
              const getOther = m => m.match.a.song === song.song ? m.match.b : m.match.a;
              let cmp = 0;
              if (matchSort.key === 'az') cmp = getOther(a).song.localeCompare(getOther(b).song);
              else if (matchSort.key === 'bpm') cmp = a.match.bpmDiff - b.match.bpmDiff;
              else if (matchSort.key === 'key') cmp = getOther(a).key.localeCompare(getOther(b).key);
              else if (matchSort.key === 'score') cmp = (b.match.score ?? -1) - (a.match.score ?? -1);
              return matchSort.dir === 'asc' ? cmp : -cmp;
            });
            const pinnedPairKey = typeof selectedMatch === 'string' && selectedMatch.startsWith('pair:')
              ? selectedMatch.slice(5) : null;
            const pinnedEntry = pinnedPairKey ? sortedGroupMatches.find(e => e.pairKey === pinnedPairKey) : null;
            return (
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <div style={{ flexShrink: 0 }}>
                  <GroupHeader
                    song={song}
                    count={sortedGroupMatches.length}
                    expanded={true}
                    highlighted={highlightedSong?.song === song.song}
                    onToggle={() => { setExpandedGroups(prev => ({ ...prev, [song.song]: false })); setHighlightedSong(null); setSelectedMatch(null); }}
                    onTitleClick={() => {}}
                    sortControls={{ matchSort, toggleMatchSort: k => toggleSort(matchSort, k, setMatchSort) }}
                    onArtistFilter={artist => { setArtistFilter(prev => prev === artist ? null : artist); if (songListRef.current) songListRef.current.scrollTo({ top: 0, behavior: 'smooth' }); }}
                    artistFilter={artistFilter}
                  />
                </div>
                {/* Pinned selected match — fixed above scroll */}
                {pinnedEntry && (
                  <div style={{ flexShrink: 0, borderBottom: '1px solid #1a1a22', paddingBottom: 8 }}>
                    <MatchRow
                      match={pinnedEntry.match}
                      anchor={song}
                      pinned={true}
                      selected={true}
                      onClick={() => setSelectedMatch(null)}
                ...
                <div style={{ flex: 1, overflowY: 'auto' }}>
                  {sortedGroupMatches.map((entry) => {
                    if (entry.pairKey === pinnedPairKey) return null;
                    return (
                      <MatchRow
                        key={entry.pairKey}
                        match={entry.match}
                        anchor={song}
                        selected={false}
                        onClick={() => setSelectedMatch('pair:' + entry.pairKey)}
                        onNavigate={selectSong}
                        onScrollTo={scrollToSong}
                        onJumpToKey={jumpToKey}
                        onArtistFilter={artist => { setArtistFilter(prev => prev === artist ? null : artist); if (songListRef.current) songListRef.current.scrollTo({ top: 0, behavior: 'smooth' }); }}
          ...
          {/* Match rows (collapsed groups list — only shown when no group is expanded) */}
          {!selectedSong && Object.values(expandedGroups).some(Boolean) ? null : <div ref={matchListRef} style={{ flex: 1, overflowY: 'auto' }}>
            {songs.length === 0 ? (
              <div style={{
              ...
              // All matches mode: grouped by anchor song — only collapsed groups shown here
              (() => {
                const groupMap = new Map();
                displayedMatches.forEach(match => {
                  [match.a, match.b].forEach(anchor => {
                ...
                return groups
                  .filter(({ song }) => expandedGroups[song.song] !== true)
                  .map(({ song, matches: groupMatches }) => (
                    <div key={song.song}>
                      <GroupHeader
                        song={song}
                        ...
                        expanded={false}
                        highlighted={highlightedSong?.song === song.song}
                        onToggle={() => setExpandedGroups(prev => ({ ...prev, [song.song]: true }))}
                        onTitleClick={() => {
                          const isHighlighted = highlightedSong?.song === song.song;
                          setHighlightedSong(isHighlighted ? null : song);
                          if (!isHighlighted) {
                            if (songItemRefs.current[song.song]) {
                              songItemRefs.current[song.song].scrollIntoView({ block: 'start', behavior: 'smooth' });
                            }
                            setExpandedGroups(prev => ({ ...prev, [song.song]: true }));
                          }
                        }}
                        onArtistFilter={artist => { setArtistFilter(prev => prev === artist ? null : artist); if (songListRef.current) songListRef.current.scrollTo({ top: 0, behavior: 'smooth' }); }}
                        artistFilter={artistFilter}
